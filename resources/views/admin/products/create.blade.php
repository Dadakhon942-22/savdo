        ...
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="price" id="price" required step="0.01" value="{{ old('price') }}" class="w-full border rounded px-4 py-2">
                </div>
                <div>
                    <label for="stock" class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.stock') }} *</label>
                    <input type="number" name="stock" id="stock" required value="{{ old('stock', 0) }}" class="w-full border rounded px-4 py-2">
                </div>
            </div>
