            ...
            <div class="group relative w-full h-full">
                ...
                <a href="{{ route('products.show', $product) }}" class="block">
                    ...
                    <div class="p-4 md:p-6">
                        ...
                        <div class="mb-3 md:mb-4">
                            ...
                            <div class="mt-2">
                                <p class="text-xs md:text-sm font-extrabold text-red-600 dark:text-red-400 flex items-center">
                ...
                @auth
                    @if(auth()->user()->isAdmin())
                        <!-- Admin uchun CRUD tugmalari -->
                        <div class="px-4 md:px-6 pb-4 md:pb-6 flex gap-3">
                            <a href="{{ route('admin.products.edit', $product) }}" class="flex-1 bg-gradient-to-r from-blue-500 via-blue-600 to-purple-600 hover:from-blue-600 hover:via-blue-700 hover:to-purple-700 text-white py-3 md:py-4 px-4 md:px-6 rounded-xl font-extrabold shadow-2xl hover:shadow-3xl transform hover:scale-[1.05] transition-all duration-300 flex items-center justify-center text-sm md:text-base">
                                <svg class="w-5 h-5 md:w-6 md:h-6 mr-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                                {{ __('messages.edit') }}
                            </a>
                            <form action="{{ route('admin.products.destroy', $product) }}" method="POST" class="flex-1" onsubmit="return confirm('{{ __('messages.delete_confirm') }}')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-full bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white py-3 md:py-4 px-4 md:px-6 rounded-xl font-extrabold shadow-2xl hover:shadow-3xl transform hover:scale-[1.05] transition-all duration-300 flex items-center justify-center text-sm md:text-base">
                                    <svg class="w-5 h-5 md:w-6 md:h-6 mr-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    @else
                        <!-- Oddiy foydalanuvchilar uchun "Savatchaga qo'shish" -->
                        @php
    ...
    @if(!auth()->check() || !auth()->user()->isSeller())
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 w-full" data-grid-responsive>
            ...
            <div class="group relative w-full h-full">
                ...
                <!-- Asosiy oyna (yuqorida) -->
                <div class="relative bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:via-gray-800 dark:to-gray-900 rounded-2xl shadow-2xl hover:shadow-3xl border-2 border-gray-200 dark:border-gray-700 overflow-hidden transform transition-all duration-500 group-hover:scale-105 group-hover:-translate-y-2 h-full flex flex-col">
                <a href="{{ route('products.show', $product) }}" class="block flex-1 flex flex-col">
                    @if($product->image)
                        <div class="relative overflow-hidden h-56 bg-gradient-to-br from-gray-100 via-gray-50 to-white dark:from-gray-700 dark:via-gray-800 dark:to-gray-900 flex items-center justify-center">
                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->localized_name }}" class="max-w-full max-h-full object-contain p-2 group-hover:scale-110 transition-transform duration-500">
                        </div>
                    @else
                    ...
                    <div class="p-4 md:p-6 flex-1 flex flex-col">
                        <h3 class="font-extrabold text-xl md:text-2xl mb-3 text-emerald-600 dark:text-emerald-400 line-clamp-2 group-hover:text-emerald-700 dark:group-hover:text-emerald-300 transition-colors">
                            {{ $product->localized_name }}
                        </h3>
                        
                        <div class="flex items-center text-indigo-600 dark:text-indigo-400 text-sm md:text-base mb-3 font-extrabold">
                        ...
                        <div class="mb-4">
                            @if($product->is_sale_active)
                                <p class="text-sm md:text-base text-slate-500 dark:text-slate-400 line-through font-bold mb-1">
                                    {{ number_format($product->price, 0, ',', ' ') }} {{ __('messages.currency') }}
                                </p>
                                <p class="text-3xl md:text-4xl font-extrabold text-orange-600 dark:text-orange-400">
                                    {{ number_format($product->discounted_price, 0, ',', ' ') }}
                                    <span class="text-base md:text-lg font-bold text-slate-600 dark:text-slate-400">{{ __('messages.currency') }}</span>
                                </p>
                                @if($product->remaining_sale_time)
                                <p class="text-xs md:text-sm font-extrabold text-red-600 dark:text-red-400 mt-1 flex items-center">
                                    <svg class="w-3 h-3 md:w-4 md:h-4 mr-1 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                    {{ $product->remaining_sale_time }}
                                </p>
                                @endif
                            @else
                                <p class="text-3xl md:text-4xl font-extrabold text-emerald-600 dark:text-emerald-400">
                        ...
                        <div class="mt-auto">
                            @if($product->stock > 0)
                                <div class="inline-flex items-center px-4 py-2 rounded-xl text-sm md:text-base font-extrabold bg-emerald-100 dark:bg-emerald-900/60 text-emerald-800 dark:text-emerald-200 border-2 border-emerald-300 dark:border-emerald-600 shadow-lg">
                            ...
                            @else
                                <div class="inline-flex items-center px-4 py-2 rounded-xl text-sm md:text-base font-extrabold bg-rose-100 dark:bg-rose-900/60 text-rose-800 dark:text-rose-200 border-2 border-rose-300 dark:border-rose-600 shadow-lg">
                                    <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>
                                    </svg>
                                    {{ __('messages.out_of_stock') }}
