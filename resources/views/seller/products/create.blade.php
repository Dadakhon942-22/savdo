...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <h1 class="text-3xl font-bold mb-8">{{ __('messages.new_product') }}</h1>
    <p class="text-gray-600 mb-6">{{ __('messages.shop') }}: {{ $shop->name }}</p>
    ...
    <div class="bg-white rounded-lg shadow-lg p-8">
        <form action="{{ route('seller.products.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select name="category_id" id="category_id" required class="w-full border rounded px-4 py-2 @error('category_id') border-red-500 @enderror">
            ...
            <div class="grid md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="price" id="price" required step="0.01" value="{{ old('price') }}" class="w-full border rounded px-4 py-2 @error('price') border-red-500 @enderror">
                    @error('price')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" name="stock" id="stock" required value="{{ old('stock', 0) }}" class="w-full border rounded px-4 py-2 @error('stock') border-red-500 @enderror">
                    @error('stock')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-4 p-4 bg-gradient-to-r from-accent-50 to-orange-50 dark:from-accent-900/20 dark:to-orange-900/20 rounded-lg border border-accent-200 dark:border-accent-700">
                ...
                <div class="mb-3">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_on_sale" id="is_on_sale" value="1" {{ old('is_on_sale') ? 'checked' : '' }} class="rounded text-accent-600 focus:ring-accent-500">
                ...
                <div id="discount_section" style="display: {{ old('is_on_sale') ? 'block' : 'none' }};">
                    ...
                    <input type="number" name="discount_percentage" id="discount_percentage" min="0" max="99" step="0.01" value="{{ old('discount_percentage', 0) }}" class="w-full md:w-1/2 border border-gray-300 dark:border-gray-600 rounded px-4 py-2 focus:ring-2 focus:ring-accent-500 focus:border-accent-500">
