        ...
        <div class="relative text-center py-8 md:py-12 px-4 md:px-6 rounded-2xl md:rounded-3xl bg-gradient-to-br from-primary-50 via-secondary-50 to-accent-50 dark:from-primary-900/30 dark:via-secondary-900/30 dark:to-accent-900/30 border-2 border-primary-200 dark:border-primary-700 shadow-xl">
            <div class="relative z-10">
                <h1 class="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-3 md:mb-4 bg-gradient-to-r from-primary-600 via-secondary-600 to-accent-600 dark:from-primary-400 dark:via-secondary-400 dark:to-accent-400 bg-clip-text text-transparent drop-shadow-sm">
                    ✏️ {{ __('messages.edit') }} {{ __('messages.products') }}
                </h1>
                <p class="text-base md:text-xl text-slate-700 dark:text-slate-300 font-bold">{{ __('messages.update_product_info') }}</p>
            </div>
        </div>
    </div>
            ...
            <form action="{{ route('seller.products.update', $product) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="relative bg-gradient-to-br from-primary-50 via-secondary-50 to-accent-50 dark:from-gray-900 dark:to-gray-800 rounded-xl p-6 border-2 border-primary-200 dark:border-primary-700">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="name_uz" id="name_uz" required value="{{ old('name_uz', $product->name_uz ?? $product->name) }}" class="w-full border-2 border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-gray-700 font-bold focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('name_uz') border-red-500 @enderror">
                                @error('name_uz')
                            ...
                            <div>
                                ...
                                <input type="text" name="name_en" id="name_en" required value="{{ old('name_en', $product->name_en ?? $product->name) }}" class="w-full border-2 border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-gray-700 font-bold focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('name_en') border-red-500 @enderror">
                                @error('name_en')
                                    <p class="text-red-600 dark:text-red-400 text-sm mt-2 font-extrabold">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="relative bg-gradient-to-br from-secondary-50 via-accent-50 to-primary-50 dark:from-gray-900 dark:to-gray-800 rounded-xl p-6 border-2 border-secondary-200 dark:border-secondary-700">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="description_ru" id="description_ru" rows="4" class="w-full border-2 border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-gray-700 font-medium focus:ring-2 focus:ring-secondary-500 focus:border-secondary-500 @error('description_ru') border-red-500 @enderror">{{ old('description_ru', $product->description_ru ?? $product->description) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="description_en" id="description_en" rows="4" class="w-full border-2 border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-gray-700 font-medium focus:ring-2 focus:ring-secondary-500 focus:border-secondary-500 @error('description_en') border-red-500 @enderror">{{ old('description_en', $product->description_en ?? $product->description) }}</textarea>
                ...
                <div class="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="number" name="price" id="price" required step="0.01" value="{{ old('price', $product->price) }}" class="w-full border-2 border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-gray-700 font-extrabold text-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('price') border-red-500 @enderror">
                        @error('price')
                            <p class="text-red-600 dark:text-red-400 text-sm mt-2 font-extrabold">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="number" name="stock" id="stock" required value="{{ old('stock', $product->stock) }}" class="w-full border-2 border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-gray-700 font-extrabold text-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('stock') border-red-500 @enderror">
                        @error('stock')
                            <p class="text-red-600 dark:text-red-400 text-sm mt-2 font-extrabold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="relative bg-gradient-to-r from-orange-50 via-red-50 to-pink-50 dark:from-orange-900/30 dark:via-red-900/30 dark:to-pink-900/30 rounded-xl p-6 border-2 border-orange-200 dark:border-orange-700">
                        ...
                        <div class="mb-4">
                            <label class="flex items-center cursor-pointer p-3 bg-white dark:bg-gray-800 rounded-xl border-2 border-gray-200 dark:border-gray-700 hover:border-orange-300 dark:hover:border-orange-600 transition-colors">
                                <input type="checkbox" name="is_on_sale" id="is_on_sale" value="1" {{ old('is_on_sale', $product->is_on_sale) ? 'checked' : '' }} class="w-5 h-5 rounded text-orange-600 focus:ring-orange-500">
                        ...
                        <div id="discount_section" style="display: {{ old('is_on_sale', $product->is_on_sale) ? 'block' : 'none' }};">
                            <div class="mb-4">
                                ...
                                <input type="number" name="discount_percentage" id="discount_percentage" min="0" max="99" step="0.01" value="{{ old('discount_percentage', $product->discount_percentage) }}" class="w-full md:w-1/2 border-2 border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-gray-700 font-extrabold focus:ring-2 focus:ring-orange-500 focus:border-orange-500 @error('discount_percentage') border-red-500 @enderror">
                            ...
                            <div>
                                ...
                                <input type="number" name="sale_duration_days" id="sale_duration_days" value="{{ old('sale_duration_days', $product->sale_duration_days) }}" min="1" max="365" step="1" placeholder="{{ __('messages.sale_duration_placeholder') }}" class="w-full md:w-1/2 border-2 border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-slate-900 dark:text-white bg-white dark:bg-gray-700 font-extrabold focus:ring-2 focus:ring-orange-500 focus:border-orange-500 @error('sale_duration_days') border-red-500 @enderror">
                ...
                <div class="relative mb-6">
                    ...
                    <div class="relative bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:to-gray-800 rounded-xl p-6 border-2 border-blue-200 dark:border-blue-700">
                        ...
                        <div class="mb-4">
                            ...
                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->localized_name }}" id="current-image" class="h-32 w-32 object-cover rounded-xl border-2 border-gray-300 dark:border-gray-600 shadow-md">
                        </div>
                        @endif
                        
                        <div>
                            <label for="image" class="block text-base font-bold text-slate-900 dark:text-white mb-2">{{ __('messages.new_image') }}</label>
                        ...
                        if (file) {
                            ...
                            reader.onload = function(e) {
                                ...
                                if (document.getElementById('current-image')) {
                                    document.getElementById('current-image').classList.add('opacity-50');
                                }
                                @endif
                            };
                            ...
                            @if($product->image)
                            if (document.getElementById('current-image')) {
                                document.getElementById('current-image').classList.remove('opacity-50');
                            }
                            @endif
                        }
                    });
                ...
                <div class="mb-6 p-4 bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-900/30 dark:to-green-900/30 rounded-xl border-2 border-emerald-200 dark:border-emerald-700">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded text-emerald-600 focus:ring-emerald-500">
